...
public class TravellerDao {
    ...
    String insertQuery = "INSERT INTO traveller (first_name, last_name, email, address, phone_number, username, password, profile_picture) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<BookingData> getAllBookingDetailsWithImage(){
        ...
        String query = "SELECT t.traveller_ID, t.first_name, t.profile_picture, b.booking_ID, b.drop_address, b.departure_date_time " +
                      "FROM traveller t " +
...
//                wrapping the data in model
    ...
    public TravellerData getTravellerById(int travellerId) {
        String query = "SELECT * FROM traveller WHERE traveller_ID = ?";
        Connection conn = mysql.openConnection();
        try {
            PreparedStatement stmnt = conn.prepareStatement(query);
